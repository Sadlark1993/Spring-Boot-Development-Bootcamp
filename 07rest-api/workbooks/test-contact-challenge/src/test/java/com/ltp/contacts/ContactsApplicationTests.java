...
class ContactsApplicationTests {
	...
	public void getContactByIdTest() throws Exception {
		...
		mockMvc.perform(request)
			...
			.andExpect(content().contentType(MediaType.APPLICATION_JSON)) //tests if the return type is Json
			.andExpect(jsonPath("$.name").value("Jon Snow")) //expects the name to be 'Jon Snow'
	...
	public void getAllContactsTest() throws Exception {
		...
		mockMvc.perform(request)
			...
			.andExpect(jsonPath("$.size()").value(3)) //asserting that the number of elements in the list is 3
			//asserts that an element with the given attributes exists in the Json file
